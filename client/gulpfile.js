...
const filesForSassCompilation = [SRC_BASE + 'scss/**/*.scss'];
...
const filesForJsCompilation = [SRC_BASE + 'js/**/*.js'];
			...
			.pipe(
				gulpFn(function (file) {
					if (file.path.indexOf('.js.map') === -1) {
						...
						console.log(file.path.replace(SRC_BASE, DIST_BASE));
					}
				})
			)
	);
...
const filesForImageCompression = [SRC_BASE + '/img/**/*.*'];
gulp.task('image-compress', (done) => {
	let imgDest = DIST_BASE + '/img/';
	return gulp
		.src(filesForImageCompression) // watch these files
		.pipe(plumber())
		.pipe(changed((file) => file.base.replace(SRC_BASE, DIST_BASE)))
		.pipe(imagemin())
		.pipe(gulp.dest((file) => file.base.replace(SRC_BASE, DIST_BASE))) // put the image files here.
		.pipe(browserSync.stream()) // tell browsersync to send over the changes
		.pipe(
			gulpFn(function (file) {
